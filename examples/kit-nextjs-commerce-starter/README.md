...
Sitecore Pages uses `/` via the catch-all route. Local commerce diagnostics live at `/test` (alias `/oc-test`): shopper auth and checkout configuration against OrderCloud sandbox. Use `/products` and `/cart` for storefront behavior.
